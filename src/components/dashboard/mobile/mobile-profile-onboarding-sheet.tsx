...
export function MobileProfileOnboardingSheet({ profileId, onClose, onDelete, editMode, showUploadStep, onSaved }: MobileProfileOnboardingSheetProps) {
  ...
  const inputClass = (filled: boolean) =>
    `w-full h-12 px-4 text-sm rounded-xl border text-white placeholder-white/30 focus:outline-none focus:border-[var(--color-accent)] transition-colors ${filled ? "bg-white/5 border-white/10" : "bg-yellow-500/10 border-yellow-500/30"}`;

  return (
    <div className="fixed inset-0 z-[200] flex flex-col justify-end">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <div
        className="relative bg-[#1C1C1E] rounded-t-[24px] flex flex-col overflow-hidden"
        style={{ height: "calc(100dvh - 3rem)", transform: `translateY(${dragY > 0 ? dragY : 0}px)`, paddingBottom: "env(safe-area-inset-bottom, 0px)" }}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          ...
          {step === "upload" && (
            <div onClick={() => inputRef.current?.click()}
              className="flex flex-col items-center justify-center gap-3 p-10 rounded-2xl border-2 border-dashed border-white/10 hover:border-[var(--color-accent)]/40 cursor-pointer transition-colors">
              <div className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center">
                <Upload size={22} className="text-white/60" />
              </div>
              <p className="text-sm text-white/80 text-center">Upload your CVs (PDF or DOCX) to auto-fill your profile</p>
          ...
          {step === "form" && (
            <div className="flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-2">
                  {jobTitles.map((t, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <input value={t} onChange={(e) => { const next = [...jobTitles]; next[i] = e.target.value; setJobTitles(next); }} placeholder="e.g. Software Engineer" className="flex-1 h-12 px-4 text-sm rounded-xl bg-white/5 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:border-[var(--color-accent)]" />
                      {jobTitles.length > 1 && <button onClick={() => setJobTitles(jobTitles.filter((_, idx) => idx !== i))} className="w-11 h-11 flex items-center justify-center text-white/50 hover:text-red-400"><X size={16} /></button>}
                    </div>
                  ))}
                </div>
                <button onClick={() => setJobTitles([...jobTitles, ""])} className="mt-2 text-xs text-[var(--color-accent)]">+ Add another title</button>
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-white/50">City or country only.</p>
              </div>

              <div>
                <label className="text-sm font-medium text-white mb-1.5 block">Industry Ladder <span className="text-white/50 font-normal">(AI-populated)</span></label>
                  ...
                  {[
                    ...
                    <div key={s.label}>
                      ...
                      <input value={s.value} onChange={(e) => s.setter(e.target.value)} placeholder="e.g. Banking"
                        className="w-full h-10 px-3 text-sm rounded-lg bg-white/5 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:border-[var(--color-accent)]" />
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="flex gap-2">
                  {["On-site", "Hybrid", "Remote"].map((type) => (
                    <button key={type} type="button" onClick={() => setJobTypes((prev) => prev.includes(type) ? prev.filter((t) => t !== type) : [...prev, type])}
                      className={`flex-1 h-12 flex items-center justify-center rounded-xl text-sm border transition-colors ${jobTypes.includes(type) ? "bg-[var(--color-accent)]/20 border-[var(--color-accent)] text-white" : "bg-white/5 border-white/10 text-white/70"}`}>
                      {type}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="text-sm font-medium text-white mb-1.5 block">CV Variations <span className="text-white/50 font-normal">(max 4)</span></label>
                <div className="flex flex-col gap-2">
                    ...
                    <div key={i} className="p-3 rounded-xl bg-white/5 border border-white/10">
                      <div className="flex items-center gap-2">
                        ...
                        {labellingPaths.has(cv.file_path) ? (
                          <div className="flex-1 flex items-center gap-2 px-2 py-1.5"><Loader2 size={12} className="text-white/40 animate-spin" /><span className="text-sm text-white/40">Identifying...</span></div>
                        ) : (
                          <input value={cv.name} onChange={(e) => { const next = [...cvVariations]; next[i] = { ...next[i], name: e.target.value }; setCvVariations(next); }}
                            placeholder="e.g. General, Tech Focus" className="flex-1 px-2 py-1.5 text-sm rounded-lg bg-white/5 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:border-[var(--color-accent)]" />
                        )}
                      </div>
                      <div className="flex items-center gap-2 text-xs text-white/60 mt-1.5 pl-6">
                        ...
                        {cvVariations.length > 1 && <button onClick={() => setCvVariations((prev) => prev.filter((_, idx) => idx !== i))} className="text-red-400"><X size={13} /></button>}
                      </div>
                    </div>
                  ))}
                </div>
                {cvVariations.length < 4 && (
                  <>
                    ...
                    <button onClick={() => addInputRef.current?.click()} disabled={uploadingCv} className="mt-2 flex items-center gap-1.5 text-xs text-[var(--color-accent)] disabled:opacity-50">
                      {uploadingCv ? <Loader2 size={12} className="animate-spin" /> : <Plus size={12} />}
              ...
              <button onClick={handleSave} disabled={saving}
                className="w-full h-12 flex items-center justify-center gap-2 rounded-xl bg-[var(--color-accent)] text-white text-sm font-medium disabled:opacity-50">
                ...
                <div className="pt-3 border-t border-white/[0.06]">
                  {confirmDelete ? (
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={handleDelete} className="h-10 px-4 text-xs font-medium text-white bg-red-500/80 rounded-lg">Confirm</button>
                      <button onClick={() => setConfirmDelete(false)} className="h-10 px-4 text-xs font-medium text-white/80">Cancel</button>
                    </div>
                  ) : (
                    <button onClick={() => setConfirmDelete(true)} className="flex items-center gap-1.5 text-xs text-red-400">
