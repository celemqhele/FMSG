...
export function MobileLimitModal({ code, plan, onClose, onOpenPfModal }: MobileLimitModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end transition-opacity duration-300" style={{ opacity: mounted ? 1 : 0 }}>
      ...
      <div
        className="relative w-full bg-[var(--color-error)] rounded-t-[19px] p-5 text-center space-y-3 safe-area-bottom transition-all duration-300 ease-out"
        style={{
          opacity: mounted ? 1 : 0,
          transform: mounted ? "translateY(0)" : "translateY(20px)",
        }}
      >
        <button onClick={onClose} className="absolute top-2.5 right-2.5 p-2 text-white/60 hover:text-white">
          <X size={14} />
        </button>
        ...
        <div className="flex justify-center gap-2.5">
          <button
            ...
            className="px-5 py-2 text-[11px] font-semibold text-[var(--color-error)] bg-white rounded-full"
          >
            Top Up
          </button>
          {code === "LIMIT_003" && (
            <button
              ...
              className="px-5 py-2 text-[11px] font-medium text-white border border-white/50 rounded-full"
            >
              Buy PF
            </button>
          )}
