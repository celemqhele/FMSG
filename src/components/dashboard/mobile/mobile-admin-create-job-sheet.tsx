...
export function MobileAdminCreateJobSheet({ isOpen, onClose }: MobileAdminCreateJobSheetProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col justify-end">
      ...
      <div
        className="relative bg-[#1C1C1E] rounded-t-[24px] overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pb-3 shrink-0 border-b border-white/[0.06]">
          <h2 className="text-lg font-semibold text-white">Create Job Post</h2>
          <button onClick={handleClose} className="w-11 h-11 flex items-center justify-center text-white/60 hover:text-white">
            <X size={20} />
          </button>
        </div>
        ...
        {published.length > 0 ? (
          <div className="flex-1 overflow-y-auto p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <p className="text-sm font-medium text-white">{published.length} job post{published.length > 1 ? "s" : ""} created</p>
            </div>
              ...
              {published.map((p) => (
                <div key={p.slug} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/10">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-white/50 font-mono">{p.url}</p>
                  </div>
                  <button
                    onClick={() => navigator.clipboard.writeText(`${window.location.origin}${p.url}`)}
                    className="ml-3 px-3 py-1.5 text-xs font-medium text-indigo-400 bg-indigo-500/10 rounded-lg shrink-0"
                  >
                    Copy
                  </button>
                </div>
            ...
            <button onClick={reset} className="mt-4 w-full h-12 flex items-center justify-center text-sm font-medium text-indigo-400 border border-indigo-500/30 rounded-xl">
              Create Another
            </button>
          </div>
        ) : (
          <>
            <div className="flex border-b border-white/[0.06] shrink-0">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 h-12 text-sm font-medium transition-colors ${tab === "paste" ? "text-[var(--color-accent)] border-b-2 border-[var(--color-accent)]" : "text-white/50"}`}
              >
                <Link size={14} /> Paste Link
              </button>
              <button
                onClick={() => setTab("search")}
                className={`flex-1 flex items-center justify-center gap-2 h-12 text-sm font-medium transition-colors ${tab === "search" ? "text-[var(--color-accent)] border-b-2 border-[var(--color-accent)]" : "text-white/50"}`}
              >
            ...
            <div className="flex-1 overflow-y-auto p-5">
              ...
              {tab === "paste" && !scrapedJob && (
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-white mb-1.5">Job URL</label>
                    <input type="url" value={pasteUrl} onChange={(e) => setPasteUrl(e.target.value)} placeholder="https://linkedin.com/jobs/..."
                      className="w-full h-12 px-4 text-sm text-white placeholder-white/30 border border-white/10 rounded-xl bg-white/5 focus:outline-none focus:border-[var(--color-accent)]" />
                  </div>
                  <button onClick={handleScrape} disabled={loading || !pasteUrl.trim()}
                    className="w-full h-12 flex items-center justify-center gap-2 text-sm font-medium text-white bg-indigo-600 rounded-xl disabled:opacity-50">
              ...
              {tab === "paste" && scrapedJob && (
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-medium text-white/50 mb-1">Job Title</label>
                    <input type="text" value={scrapedJob.job_title} onChange={(e) => setScrapedJob({ ...scrapedJob, job_title: e.target.value })}
                      className="w-full h-12 px-3 text-sm text-white border border-white/10 rounded-xl bg-white/5 focus:outline-none focus:border-[var(--color-accent)]" />
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-white/50 mb-1">Paraphrased Description</label>
                    <textarea value={scrapedJob.paraphrased_description} onChange={(e) => setScrapedJob({ ...scrapedJob, paraphrased_description: e.target.value })}
                      rows={8}
                      className="w-full px-3 py-2 text-sm text-white border border-white/10 rounded-xl bg-white/5 focus:outline-none focus:border-[var(--color-accent)] resize-none" />
                  </div>
                  <button onClick={handlePublishPaste} disabled={publishing}
                    className="w-full h-12 flex items-center justify-center gap-2 text-sm font-medium text-white bg-emerald-600 rounded-xl disabled:opacity-50">
              ...
              {tab === "search" && loading && (
                <div className="flex flex-col items-center gap-4 py-12">
                  ...
                  <div className="text-center">
                    <p className="text-sm font-medium text-white">Searching high-demand jobs in Gauteng...</p>
                    <p className="text-xs text-white/50 mt-1">AI is selecting the top 5 most popular roles</p>
                  </div>
                </div>
              )}
              ...
              {tab === "search" && !loading && searchResults.length > 0 && (
                <div className="space-y-4">
                  <p className="text-xs text-white/50">AI picked these {searchResults.length} high-application jobs. Uncheck any you don&apos;t want, then publish.</p>
                  <div className="space-y-2">
                    {searchResults.map((r, i) => (
                      <button key={i} onClick={() => toggleResult(i)}
                        className={`w-full text-left p-3 rounded-xl border transition-colors ${r.selected ? "border-indigo-500 bg-indigo-500/10" : "border-white/10 bg-white/5"}`}>
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-white truncate">{r.title}</p>
                            <p className="text-xs text-white/50">{r.company_name}, {r.location || "Gauteng"}</p>
                          </div>
                          <div className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 mt-0.5 ${r.selected ? "border-indigo-500 bg-indigo-500" : "border-white/30"}`}>
              ...
              {tab === "search" && !loading && searchResults.length === 0 && !error && (
                <div className="text-center py-12"><p className="text-sm text-white/50">No results found. Try again later.</p></div>
              )}
            </div>
          </>
        )}
